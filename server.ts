...
app.post("/api/flows", async (req: Request<unknown, unknown, FlowInput>, res, next) => {
  if (!req.body.name || !req.body.cost || !req.body.category || !req.body.bankAccount || !req.body.date) {
    res.status(400).send("Missing parameter");
    return;
  }
  let query: { insertId: number | undefined };
  try {
    query = await db.query("INSERT INTO `flows` (`id`, `name`, `cost`, `category`, `bank_account`, `date`) VALUES (NULL, ?, ?, ?, ?, ?)", [req.body.name, req.body.cost, req.body.category, req.body.bankAccount, req.body.date]);
  } catch (err) {
    return next(err);
  }
...
app.post("/api/transfers", async (req: Request<unknown, unknown, TransferInput>, res, next) => {
  if (!req.body.name || !req.body.amount || !req.body.rate || !req.body.category || !req.body.fromAccount || !req.body.toAccount || !req.body.date) {
    res.status(400).send("Missing parameter");
    return;
  }
  let query: { insertId: number | undefined };
  try {
    query = await db.query("INSERT INTO `transfers` (`name`, `amount`, `rate`, `category`, `from_account`, `to_account`, `date`) VALUES (?, ?, ?, ?, ?, ?, ?)", [req.body.name, req.body.amount, req.body.rate, req.body.category, req.body.fromAccount, req.body.toAccount, req.body.date]);
  } catch (err) {
    return next(err);
  }
  res.status(200).send(String(query.insertId));
